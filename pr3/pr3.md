...
1.  Напишите функцию `sequence`​, которая принимает аргументы `low`​, `high` ​​(целые числа) и `step` ​(положительное число). Функция `sequence` ​должна возвращать список чисел от `low` ​до `high` с ​шагом `step`​. Например:
...
3. Рассмотрим вложенный список, похожий на тот, что рассмотрен выше, но кроме чисел допускаются элементы другого типа.
